...
import {
  ...
  itemQuantityUp,
  removeFromCart,
} from "../redux/features/food/cartSlice";
...
function FoodCard2({ id, name, price, img, quantity, cart = false }) {
  ...
  return (
    <div
      onClick={foodDetails}
      className="rounded-lg w-full bg-white p-1 h-full  flex justify-between max-h-32 sm:max-h-36 "
    >
      {/*============== Card Content ==============*/}
          ...
          <div>
            {cart ? (
              <div className="flex items-center  gap-1 text-gray-800  w-full pt-2 ">
                <button
                  onClick={decreaseItemQuantiy}
                  className="border-[1px] border-gray-400 px-2  text-2xl md:text-3xl text-gray-400"
                >
                  <span>-</span>
                </button>
                <span className="px-2 font-semibold md:text-lg">
                  {quantity}
                </span>
                <button
                  onClick={increaseItemQuantiy}
                  className="border-[1px] border-red-500 px-2 text-red-500 text-2xl md:text-3xl"
                >
                  <span>+</span>
                </button>
              </div>
